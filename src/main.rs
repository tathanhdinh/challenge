...
fn western_cell(x: usize, y: usize,
    ...
    let lazy_value_for_true: Box<Fn(usize) -> Option<NodeIndex>> = Box::new(|i: usize| Some(cell_matrix[y][i - 1]));
    let lazy_value_for_false: Box<Fn(usize) -> Option<NodeIndex>> = Box::new(|_| None);
    // let lazy_value = select_generic::<_>(x > 0, &lazy_value_for_true, &lazy_value_for_false);
    // lazy_value(x)
    select_generic::<_>(x > 0, &lazy_value_for_true, &lazy_value_for_false)(x)
    // let dec = |x: usize| { x - 1 };
